...
def import_enemy_spritesheet(path, width, height):
    ...
    for i in range(int(wid / width)):
        ...
        # Attack 1
        line = pygame.Surface((width, height)).convert_alpha()
        line.blit(sprite_sheet, (0, 0), (i * width, height, width, height))
        line.set_colorkey((0, 0, 0))
        animation_library["attack1"].append(line)

        # Run
        line = pygame.Surface((width, height)).convert_alpha()
        line.blit(sprite_sheet, (0, 0), (i * width, 2 * height, width, height))
        line.set_colorkey((0, 0, 0))
        animation_library["run"].append(line)

        # Attack 2
        line = pygame.Surface((width, height)).convert_alpha()
        line.blit(sprite_sheet, (0, 0), (i * width, 3 * height, width, height))
        line.set_colorkey((0, 0, 0))
        animation_library["attack2"].append(line)

        # Dead
        line = pygame.Surface((width, height)).convert_alpha()
        line.blit(sprite_sheet, (0, 0), (i * width, 4 * height, width, height))
        line.set_colorkey((0, 0, 0))
        animation_library["dead"].append(line)

    return animation_library

def import_boss_spritesheet(path, width, height):
    animation_library = {"idle": [], "attack1": [], "attack2": [], "run": [], "push": [], "dead": []}
    ...
    for i in range(int(wid / width)):
        ...
        # Dead
        line = pygame.Surface((width, height)).convert_alpha()
        line.blit(sprite_sheet, (0, 0), (i * width, 0 * height, width, height))
        line.set_colorkey((0, 0, 255))
        animation_library["dead"].append(line)

        # Idle
        line = pygame.Surface((width, height)).convert_alpha()
        line.blit(sprite_sheet, (0, 0), (i * width, 1 * height, width, height))
        line.set_colorkey((0, 0, 255))
        animation_library["idle"].append(line)
        ...
        line.blit(sprite_sheet, (0, 0), (i * width, 4 * height, width, height))
        line.set_colorkey((0, 0, 255))
        animation_library["push"].append(line)

        # Attack 2
        line = pygame.Surface((width, height)).convert_alpha()
        line.blit(sprite_sheet, (0, 0), (i * width, 5 * height, width, height))
        line.set_colorkey((0, 0, 255))
        animation_library["attack2"].append(line)
